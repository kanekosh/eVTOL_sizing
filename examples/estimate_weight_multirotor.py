...
In this example, I will find a minimum-weight hexarotor design.
I require the hexarotor to carry a 2 kg payload, fly 5000 m, and hover for 240 sec. Hovering time includes the times for vertical clibm and descent (if you climb/descent slowly, you can approximate climb/descent as hover).
Optimization variables are: UAV total weight (including the payload), rotor radius, cruise speed, and rotor advance ratio in cruise.
"""
...
if __name__ == '__main__':
    ...
    uav_params = {}
    uav_params['config'] = 'multirotor'
    # rotor parameters
    uav_params['n_rotors_lift'] = 6   # number of lifting rotors
    ...
    indeps.add_output('UAVs|speed', 5., units='m/s')   # cruise speed
    indeps.add_output('UAVs|rotor_radius_lift', 0.1, units='m')   # lifting rotor radius
    indeps.add_output('rotor|mu', 0.2, units=None)  # edgewise advance ratio in cruise
    ...
    prob.model.add_design_var('rotor|mu', lower=0.01, upper=0.5)  # edgewise advance ratio in cruise   
    # constraints
    ...
    prob.model.add_constraint('rotor|Ct', lower=0.0, upper=0.14 * uav_params['rotor_lift_solidity'], ref=0.01)   # in cruise. CT / solidity <= 0.14 to avoid too high blade loading
